...
Db.prototype.open = function(callback) {
  ...
  if(self.serverConfig instanceof Server) {
    ...
    setTimeout(function() {
        if (!foundMaster) {
            self.emit('error', new Error('No master found'))
        }
    }, 1000)
...
Db.prototype.executeCommand = function(db_command, callback) {
    ...
    if (this.serverConfig.masterConnection.connection.readyState == 'writeOnly') {
        this.emit('error', new Error('lost connection to ' + this.serverConfig.masterConnection.host +
                  ':' + this.serverConfig.masterConnection.port))
        return
    }
